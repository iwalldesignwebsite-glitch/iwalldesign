...
interface CompareProps {
  ...
  className?: string;
  firstImageClassName?: string;
  secondImageClassname?: string; // zostawiam zgodnie z Twoim API
  initialSliderPercentage?: number;
  slideMode?: "hover" | "drag";
  showHandlebar?: boolean;
  autoplay?: boolean;
  ...
  firstAlt?: string;
  secondAlt?: string;
  priorityFirst?: boolean; // opcjonalnie LCP, jeśli ten komponent jest nad foldem
}
...
export const Compare = ({
  ...
  return (
    <div
      ref={sliderRef}
      className={cn(
        // szerokość 650px u Ciebie – ważne dla `sizes`
        "relative w-[650px] aspect-square overflow-hidden",
        className
      )}
    ...
    >
      ...
      <AnimatePresence initial={false}>
        <motion.div
          className="h-full w-[5px] absolute top-0 m-auto bg-white z-30"
      ...
      {/* DRUGIE ZDJĘCIE (pełne tło) */}
      <AnimatePresence initial={false}>
        {secondImage && (
          <motion.div
            className={cn(
              "absolute inset-0 z-[18] select-none",
              secondImageClassname
            )}
            transition={{ duration: 0 }}
          >
            <Image
              ...
              quality={68}
              priority={priorityFirst ? false : undefined}
              draggable={false}
            />
          </motion.div>
        )}
      ...
      {/* PIERWSZE ZDJĘCIE (maskowane clipPath) */}
      <AnimatePresence initial={false}>
        {firstImage && (
          <motion.div
            className={cn(
          ...
          >
            ...
            <Image
              ...
              className="object-cover"
              quality={68}
              priority={priorityFirst || undefined}
              draggable={false}
            />
          </motion.div>
        )}
